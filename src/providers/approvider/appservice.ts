...
export class AppService {
  ...
  private readonly endOfTestTrial = moment("2019-05-27");
  constructor() { 
  ...
  public updateAdvertOnStores() {
    ...
    // const week = moment("2019-05-27").isoWeek();
  ...
  getPublicAdverts():AdvertImg[] {
    if(this.choosenStore !== undefined) {
      return this.stores.filter(s => s.publicAdvert).map(s => s.publicAdvert);
    }
    return this.getStoresInMap().filter(s => s.publicAdvert).map(s => s.publicAdvert);
  }
